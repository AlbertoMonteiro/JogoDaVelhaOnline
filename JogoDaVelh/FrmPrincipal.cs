    ...
    public partial class FrmPrincipal : Form
    {
            ...
            if (string.IsNullOrWhiteSpace(lbl.Text))
            {
                ...
                if(!online)
                {
                    if (!resultado)
                    {
                        TurnoDaMaquina();
                        if (AlguemVenceu())
                            MessageBox.Show("Não foi dessa vez, você perdeu", "Jogo da velha", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                        MessageBox.Show("Parabéns você venceu!", "Jogo da velha", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                if (DeuVelha(resultado))
                    MessageBox.Show("Empante, ninguem venceu.", "Jogo da velha", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
                MessageBox.Show("Escolha outro quadrado", "Jogo da velha", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }

        private bool DeuVelha(bool resultado)
        {
            return labels.All(x => !string.IsNullOrEmpty(x.Text)) && !resultado;
        }

        private void InformaJogada(Label lbl, bool resultado)
        {
            int i = 0, j = 0;
            bool found = false;
            for (; i < 3; i++)
        ...
        private bool ChecaLinhasOuColunas(bool ehLinha)
        {
            ...
            foreach (var item in new[] { 0, 1, 2 })
            {
                ...
                for (; i < 2; i++)
                    resultado = (ehLinha) ?
                        resultado && matriz[item, i].Text == matriz[item, i + 1].Text && !string.IsNullOrWhiteSpace(matriz[item, i].Text) :
        ...
        {
            labels.ToList().ForEach(x => x.Text = string.Empty);
            if (rbOnline.Checked)
            {
                ...
                catch (SocketException ex)
                {
                    ...
                    if (ex.SocketErrorCode == SocketError.ConnectionRefused)
                        resultado = MessageBox.Show("Não foi possível se conectar ao amigo.\nClique em repetir para tentar novamente", "Jogo da velha", MessageBoxButtons.RetryCancel, MessageBoxIcon.Error);
                    if (resultado == DialogResult.Retry)
                        IniciarJogo(sender, e);
                }
            }
            else
            {
        ...
        {
            labels.ToList().ForEach(x => x.Invoke(new Action(() => x.Enabled = liberar)));
        }
